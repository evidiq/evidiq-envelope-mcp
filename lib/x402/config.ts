...
export function getX402Config(): X402ServerConfig {
  return {
    ...
    publicBaseUrl: (process.env.PUBLIC_BASE_URL || "https://mcp.evidiq.dev/envelope").replace(/\/$/, ""),
    okxCredentials: getOkxCredentials(),
  };
}

/**
 * Phase 1 build/test bypass: when ENVELOPE_X402_BYPASS=1 or X402_BYPASS=1,
 * the payment gate is disabled and all tools return 200 (PLAN §12).
 * Phase 2 removes the bypass and enables the gate (runbook §54: delete the
 * line, never set it to 0 — a present-but-falsy variable is one truthiness
 * bug away from a free service).
 */
export function isX402Bypassed(): boolean {
  return process.env.ENVELOPE_X402_BYPASS === "1" || process.env.X402_BYPASS === "1";
}
