...
/**
 * x402 payment gate for EVIDIQ Envelope MCP.
 *
 * Phase 1 (ENVELOPE_X402_BYPASS=1 or X402_BYPASS=1): gate disabled — every tool
    ...
    if (bypassed) {
      ...
      return new Response(
        JSON.stringify({
          ...
          x402: "bypassed",
          note: "Phase 1 test build — payment gate disabled (ENVELOPE_X402_BYPASS=1). POST JSON-RPC to this endpoint.",
        }),
        { status: 200, headers: { "Content-Type": "application/json", "Cache-Control": "no-store" } }
      );
    }
    // GET /mcp returns 402 challenge (§7 of x402 runbook)
    return build402Response("verify_dkim");
  }
