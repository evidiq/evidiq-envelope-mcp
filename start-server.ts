...
import { MAILAUTH_VERSION } from "./lib/envelope/auth.js";
...
import { getSignerAddress } from "./lib/envelope/report.js";
...
const PUBLIC_BASE_URL = (process.env.PUBLIC_BASE_URL || "https://mcp.evidiq.dev/envelope").replace(/\/$/, "");
const SLUG = "envelope";

const bypassed = isX402Bypassed();
if (bypassed) {
  console.warn("[envelope] X402 GATE BYPASSED — Phase 1 test build. Paid tools answer without payment.");
}

const db = openArtifactStore(process.env.ENVELOPE_DB_PATH || "/data/envelope.db");

const handler = createEnvelopeServer(db);
...
httpServer.listen(PORT, HOST, () => {
  console.log(`[evidiq-envelope-mcp] listening at http://${HOST}:${PORT}`);
  console.log(`[evidiq-envelope-mcp] Payment Gate: ${bypassed ? "BYPASSED (TEST BUILD)" : "ENFORCED"}`);
  console.log(`[evidiq-envelope-mcp] Endpoints: /health, /x402, /skill.md, /mcp`);
});

process.on("unhandledRejection", (reason) => {
  console.error("[evidiq-envelope-mcp] unhandled rejection — staying up:", reason);
});
process.on("uncaughtException", (error) => {
  console.error("[evidiq-envelope-mcp] uncaught exception — staying up:", error);
});
